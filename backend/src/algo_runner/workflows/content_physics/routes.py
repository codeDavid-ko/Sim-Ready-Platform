"""content-physics 잡 제출 라우트 — 긴 WSL 파이프라인을 백그라운드 잡으로.

POST /api/workflows/content-physics/submit        -> {job_id}
POST /api/workflows/content-physics/render-submit  -> {job_id}  (Isaac 360° 렌더)
...
router = APIRouter(tags=["content-physics"])

_MAX_FILE = 1024 * 1024 * 1024  # 1GB
_WF_ID = "content-physics"
