  ...
  return (
    <>
      <div className="card">
        ...
        <form onSubmit={run}>
          <label>USD 파일 ({accept})<Tip t="재질을 추론할 USD를 업로드합니다. NVIDIA content-agents가 부품별로 자체 멀티뷰 렌더를 찍어 재질(MaterialX)을 배정합니다." /></label>
          <input type="file" accept={accept} onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
          <p className="muted">WSL2에서 멀티뷰 렌더 + 부품별 구독 Claude VLM 분류가 돌아 <b>수 분</b> 걸립니다.</p>
          <div style={{ marginTop: 12 }}>
            <button type="submit" disabled={busy}>{busy ? "실행 중…" : "재질 추론 실행 (NVIDIA)"}</button>
          </div>
      ...
      {result && (
        <>
          ...
          <div className="card">
            <div className="row" style={{ justifyContent: "space-between" }}>
              <label style={{ margin: 0 }}>결과 다운로드</label>
              {result.asset && <span className="badge ok">저장소 등록됨</span>}
            </div>
            <div className="row" style={{ marginTop: 8, gap: 8, flexWrap: "wrap" }}>
              {(result.usdz_asset || result.asset) && (
                <button onClick={() => { const a = result.usdz_asset ?? result.asset!; downloadFile(a.download_url, a.filename); }}>재질 USD 다운로드</button>
              )}
            </div>
          </div>
          {result.usdz_asset && (
            <div className="card">
              <label style={{ margin: 0 }}>인터랙티브 RTX 뷰어 (실제 MaterialX · 좌우 회전·상하 고도·휠 확대)</label>
              <p className="muted">결과를 USDZ로 묶어(형상+MaterialX+텍스처) Isaac Sim RTX로 렌더 후 마우스로 돌려봅니다.</p>
